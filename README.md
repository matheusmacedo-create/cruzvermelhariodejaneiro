...
- **Checkout, pendências para fechar**: (1) aviso de inscrição paga vai para
  `contato@cruzvermelhariodejaneiro.org` (`EMAIL_SECRETARIA`), confirmar o endereço da secretaria;
  (2) acesso à escola (versão A) depende da API da escola (`ESCOLA_API_URL`), que ainda não existe;
  (3) as transações de teste (R$ 1 e R$ 99, "Teste Integracao", não pagas) aparecem no painel da
  Unicopag até expirarem. Resend configurado em 18/09 (e-mail de teste entregue a `contato@`).
